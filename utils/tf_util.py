...
def conv2d(inputs,
  ...
  with tf.compat.v1.variable_scope(scope) as sc:
      ...
      outputs = tf.nn.conv2d(inputs, filters=kernel,
...
def fully_connected(inputs,
  ...
  with tf.compat.v1.variable_scope(scope) as sc:
    ...
    num_input_units = int(inputs.get_shape()[-1])
...
def dropout(inputs,
  ...
  with tf.compat.v1.variable_scope(scope) as sc:
    ...
    #                   lambda: tf.nn.dropout(inputs, noise_shape, rate=1 - (keep_prob)),
    #                   lambda: inputs)
    # 在 TensorFlow 2.x 中，tf.nn.dropout 函数的参数没有 noise_shape，而是使用 rate 来指定丢弃的概率。
    # 如果是 TensorFlow 2.x，使用 tf.keras.layers.Dropout 层
    dropout_layer = tf.keras.layers.Dropout(rate=1 - keep_prob)
    outputs = tf.cond(is_training,
                      lambda: dropout_layer(inputs, training=True),
                      lambda: inputs)
    return outputs
