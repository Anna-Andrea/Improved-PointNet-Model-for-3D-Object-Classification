...
parser.add_argument('--model', default='pointnet_cls', help='Model name: pointnet_cls_se or pointnet_cls or pointnet_cls_se_bn [default: pointnet_cls]')
...
# Save to a new log file for PoinNet_SE-block & PoinNet_SE-block_BN
if FLAGS.model == 'pointnet_cls_se':
    LOG_DIR = 'log/se-block'
elif FLAGS.model == 'pointnet_cls_se_bn':
    LOG_DIR = 'log/bn-layer'
else:
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    # 在新版本的 NumPy 中，np.float 已经被弃用了，如果你确实需要使用 NumPy 提供的浮点类型，你可以将 np.float 替换为 np.float64
